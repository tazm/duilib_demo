...
namespace DuiLib {
    ...
    typedef CDuiSize CSize;
    typedef CDuiRect CRect;
    typedef CDuiPoint CPoint;

/////////////////////////////////////////////////////////////////////////////////////
//

TCHAR gs_szMenuName[64] = {0};
...
CMenuWnd::CMenuWnd(HWND hParentWnd):
m_pOwner(NULL),
m_pLayout(),
m_xml(_T("")),
...
void CMenuWnd::OnFinalMessage(HWND hWnd)
{
    ...
    if (m_hParentWnd)
    {
        ::PostMessage(m_hParentWnd, WM_MENU_DELETE, 0, 0);
    }
}
	...
	if( event.Type == UIEVENT_BUTTONUP )
	{
		if( IsEnabled() ){
			...
			else
			{
				...
				if (CMenuWnd::GetGlobalContextMenuObserver().GetManager() != NULL)
				{
                    wcsncpy(gs_szMenuName, GetName().GetData(), 63);
// 					CDuiString* strPost = new CDuiString(GetName().GetData());
					PostMessage(CMenuWnd::GetGlobalContextMenuObserver().GetManager()->GetPaintWindow(), WM_MENUCLICK, (WPARAM)(gs_szMenuName), (LPARAM)(GetChecked() == TRUE));
// 						delete strPost;
				}
				ContextMenuParam param;
				param.hWnd = m_pManager->GetPaintWindow();
