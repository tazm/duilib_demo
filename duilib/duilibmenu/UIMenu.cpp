...
m_xml(_T(""))
{
    ...
    OutputDebugStringW(L"CMenuWnd construct");
}

CMenuWnd::~CMenuWnd()
{
    OutputDebugStringW(L"CMenuWnd destructor");
}
...
bool _DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rcControl, const RECT& rcPaint, CImageAttribute& image)
{
...
void CMenuElementUI::DrawItemIcon(HDC hDC, const RECT& rcItem)
{
	if ( m_icon.IsLoadSuccess() )
	{
		if (!(m_bCheckItem && !GetChecked()))
		{
            ...
            if (!m_icon.LoadImage(m_pManager))
                return;

            if (!IsRectEmpty(&rcDest))
            {
                CImageAttribute modifyImage = m_icon;
                modifyImage.SetDest(rcDest);
                _DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, modifyImage);
            }
            else
            {
                _DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, m_icon);
            }
		}
	}
}
...
void CMenuElementUI::DrawItemExpland(HDC hDC, const RECT& rcItem)
{
	if (m_bShowExplandIcon)
	{
        ...
        if (!IsRectEmpty(&rcDest))
        {
            CImageAttribute modifyImage = m_expandIcon;
            modifyImage.SetDest(rcDest);
            _DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, modifyImage);
        }
        else
        {
            _DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, m_expandIcon);
        }
	}
}
